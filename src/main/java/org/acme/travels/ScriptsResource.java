...
public class ScriptsResource {
    ...
    public ScriptsModelOutput createResource_scripts(@Context HttpHeaders httpHeaders, @QueryParam("businessKey") String businessKey, @javax.validation.Valid() @javax.validation.constraints.NotNull() ScriptsModelInput resource) {
        if (resource == null) {
            resource = new ScriptsModelInput();
        }
        final ScriptsModelInput value = resource;
        return org.kie.kogito.services.uow.UnitOfWorkExecutor.executeInUnitOfWork(application.unitOfWorkManager(), () -> {
            ProcessInstance<ScriptsModel> pi = process.createInstance(businessKey, mapInput(value, new ScriptsModel()));
    ...
    public List<ScriptsModelOutput> getResources_scripts() {
        return process.instances().values().stream().map(pi -> mapOutput(new ScriptsModelOutput(), pi.variables())).collect(Collectors.toList());
    }
    ...
    public ScriptsModelOutput getResource_scripts(@PathParam("id") String id) {
        return process.instances().findById(id).map(pi -> mapOutput(new ScriptsModelOutput(), pi.variables())).orElse(null);
    }
    ...
    public ScriptsModelOutput deleteResource_scripts(@PathParam("id") final String id) {
        return org.kie.kogito.services.uow.UnitOfWorkExecutor.executeInUnitOfWork(application.unitOfWorkManager(), () -> {
            ProcessInstance<ScriptsModel> pi = process.instances().findById(id).orElse(null);
            if (pi == null) {
                return null;
            } else {
                pi.abort();
    ...
    public ScriptsModelOutput updateModel_scripts(@PathParam("id") String id, ScriptsModel resource) {
        return org.kie.kogito.services.uow.UnitOfWorkExecutor.executeInUnitOfWork(application.unitOfWorkManager(), () -> {
            ...
            if (pi == null) {
                ...
                return mapOutput(new ScriptsModelOutput(), pi.variables());
            }
        });
    }
    ...
    protected ScriptsModelOutput getModel(ProcessInstance<ScriptsModel> pi) {
    ...
    protected ScriptsModel mapInput(@javax.validation.Valid() @javax.validation.constraints.NotNull() ScriptsModelInput input, ScriptsModel resource) {
        resource.fromMap(input.toMap());
        return resource;
    }

    protected ScriptsModelOutput mapOutput(ScriptsModelOutput output, ScriptsModel resource) {
